    ...
    <center>
        <div class="container">
            ...
            <form action="#" method="post" id="bakiyeForm">
                ...
                <div class="form-group">
                    <label for="bakiye">Bakiye:</label><br>
                    <input type="text" id="bakiye" name="bakiye" disabled>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Veritabanında bu TC Kimlik Numarasına sahip kullanıcıyı sorgula
    $stmt = $pdo->prepare("SELECT balance FROM users WHERE idNumber = ?");
    $stmt->execute([$tc]);
    $user = $stmt->fetch();

    // Kullanıcı varsa bakiyesini ekrana yazdır
    if ($user) {
        // JavaScript ile bakiye alanına yazdır
        echo "<script>document.getElementById('bakiye').value = '{$user['balance']}';</script>";
    } else {
